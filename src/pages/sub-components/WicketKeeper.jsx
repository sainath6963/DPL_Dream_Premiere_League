...
const WicketKeeper = () => {
  ...
  const [wicketKeepers, setWicketKeepers] = useState([]); // Renamed state variable
  ...
  useEffect(() => {
    if (messages) {
      const filteredData = messages.filter(
        (message) => message.role?.toLowerCase() === "wicket-keeper"
      );
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen text-white">
      <h1 className="text-3xl font-bold">WicketKeeper List</h1>
      <p className="mt-2 text-gray-300">
        List of all registered WicketKeepers.
      </p>
      ...
      <div className="mt-6 space-y-4">
        {wicketKeepers.length > 0 ? (
          ...
          <p className="mt-4 text-gray-400">
            No WicketKeeper registrations found.
          </p>
        )}
      </div>
    </div>
